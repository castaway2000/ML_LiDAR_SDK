...
int main (int argc, char **argv)
{
    ...
    image_transport::Publisher pub_depth_raw = it.advertise("depth_raw", 1);
    image_transport::Publisher pub_depth_color = it.advertise("depth_color", 1);
    image_transport::Publisher pub_intensity_raw = it.advertise("intensity_raw", 1);
    image_transport::Publisher pub_grey_raw = it.advertise("grey_raw", 1);
    ...
    while (ros::ok()) {
        ...
        if (lidar_ml->get_scene(scene)) {
            ...
            if (pub_grey_raw.getNumSubscribers() > 0) {
                msg_grey = cv_bridge::CvImage(std_msgs::Header(), "mono16", grey_image_raw).toImageMsg();
                pub_grey_raw.publish(msg_grey);
            }
            ...
            if (pub_depth_raw.getNumSubscribers() > 0) {
                msg_depth = cv_bridge::CvImage(std_msgs::Header(), "mono16", depth_16b).toImageMsg();
                pub_depth_raw.publish(msg_depth);
            }
            ...
            if (pub_intensity_raw.getNumSubscribers() > 0) {
                msg_intensity = cv_bridge::CvImage(std_msgs::Header(), "mono16", intensity_image_raw).toImageMsg();
                pub_intensity_raw.publish(msg_intensity);
            }
